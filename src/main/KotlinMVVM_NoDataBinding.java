...
public class KotlinMVVM_NoDataBinding extends AnAction {
    ...
    private enum CodeType {
        BaseActivity, BaseFragment, BaseNotActivity, BaseDataActivity, BaseDataFragment, BaseRecycleViewActivity, BaseRecycleViewFragment, ViewModel
    }
    ...
    private void initSelectView() {
        ...
        panelname.setBorder(BorderFactory.createTitledBorder("模块名称"));
        moduleName = new JTextField();
        moduleName.setText("请输入模块名称。如：app");
        moduleName.addFocusListener(new MyFocusListener("请输入模块名称。如：app", moduleName));
        ...
        jFrame.setTitle("一键生成KotlinMVVM、Rx代码");
     ...
     * @return
     */
    private String readPackageName(String module) {
        ...
        try {
            ...
            Document doc = db.parse(project.getBasePath() + "/" + module + "/src/main/AndroidManifest.xml");
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getActionCommand().equals("取消")) {
                ...
                if (rxNetCodeJB.isSelected()) {
                    ...
                    rxCodeBuild(3);
                } else if ("请输入模块名称。如：app".equals(moduleName.getText())) {
                    Messages.showErrorDialog(project, "请输入模块名称。如：app", "提示");
                    ...
                    jFrame.dispose();
                    clickCreateFile();
                    showNotification("implementation", "Success", "Mvvm一键生成代码成功！");
                }
            }
        }
    };
    ...
    private void clickCreateFile() {
        if (baseActivityJB.isSelected()) {
            createFiles(CodeType.ViewModel);
    ...
    private void rxCodeBuild(int type) {
        ...
        switch (type) {
            case 1:
                //生成网络请求代码
                copyString = "rxLifeScope.launch({\n" +
                        "            val test = RxHttp.get(\"/service/...\")\n" +
                        "                .add(\"key\",\"\")\n" +
                        "                .addHeader(\"header\",\"\")\n" +
                        "                .toResponse<List<Test>>()\n" +
                        "                .await()\n" +
                        "        }, {\n" +
            ...
            case 2:
                //生成文件上传代码
                copyString = "rxLifeScope.launch({\n" +
                        "            val test = RxHttp.postForm(\"/service/...\")\n" +
                        "                .add(\"key\",\"\")\n" +
                        "                .addHeader(\"header\",\"\")\n" +
            ...
            case 3:
                ...
                copyString = "rxLifeScope.launch({\n" +
                        ...
                        "            val test = RxHttp.postForm(\"/service/...\")\n" +
                        "                .setRangeHeader(1000, 300000)   //如果需要断点下载，设置下载起始/结束位置\n" +
    ...
    private void createFiles(CodeType codeType) {
        ...
        packagebase = readPackageName(module);
        String packagepath = packagebase.replace(".", "/");
        String apppath = project.getBasePath() + "/" + module + "/src/main/java/" + packagepath + "/" + packageName.getText().replace(".", "/");
        modelPath = project.getBasePath() + "/" + module + "/src/main/java/" + packagepath + "/model";
        adapterPath = project.getBasePath() + "/" + module + "/src/main/java/" + packagepath + "/adapter";
        switch (codeType) {
            case BaseActivity:
            ...
            case BaseRecycleViewActivity:
                ...
                //处理
                content = dealActivity(content);
                writetoFile(content, apppath, name.getText() + "ViewModel.kt");
